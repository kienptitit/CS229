...
def init_centroids(num_clusters, image):
    ...
    # *** START YOUR CODE ***
    k = image.shape[-1]
    centroids_init = np.random.rand(num_clusters, k)
    # *** END YOUR CODE ***

    return centroids_init


def converege(label, new_label):
    if new_label is None:
        return False
    return np.equal(label, new_label)
...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    it = 0
    k = len(centroids)
    dist = np.linalg.norm(image[:, None, :] - centroids[None, :, :], axis=-1)
    label = np.argmax(dist, axis=-1)
    new_label = None
    while not converege(label, new_label) or it < max_iter:
        dist = np.linalg.norm(image[:, None, :] - centroids[None, :, :], axis=-1)
        lagbel = new_label
        new_label = np.argmax(dist, axis=-1)
        centroids = [np.mean(image[label == i], axis=0) for i in range(k)]
